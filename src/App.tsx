...
const TunerPage = lazy(() => import('./tuner/TunerPage'))

type Route = 'metronome' | 'chords' | 'jam' | 'tuner'

function readRoute(): Route {
  const value = window.location.hash.replace('#/', '')
  return value === 'chords' || value === 'jam' || value === 'tuner' ? value : 'metronome'
}
...
export default function App() {
  ...
  return <div className={`app-shell app-shell--${route}`}>
    ...
    </> : <Suspense fallback={<main className="tool-loading">正在装载练习工具…</main>}>{route === 'chords' ? <ChordPage /> : route === 'tuner' ? <TunerPage /> : <JamPage />}</Suspense>}
  </div>
}
