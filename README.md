...
- 内置 18 首常见吉他练习曲目，可按难度和风格筛选，并一键应用建议起始或目标速度与拍号。
...
- 常用和弦提供开放把位与可移动 A 型、E 型或 Power Chord 把位，可切换后单独试听。
- 和弦切换训练可配置两个和弦、40–200 BPM、每和弦 2/4/8 拍和 1/3/5 分钟练习时长。

### Jam Loop

- 4/4、3/4 拍号，40–220 BPM，以及关闭、1 小节或 2 小节预备拍。
- Rock、Pop、Ballad、Shuffle 四种伴奏风格。
- A/B/C 多段落编排；每段可设 4、8、12 小节、独立和弦进行及段尾鼓 Fill。
- 和弦进行编辑、整体升降半音、常用进行和本地自动保存，并自动迁移旧版单段 Jam 数据。
- 可选 12 个调性和大/小调模式，实时给出五声音阶、完整调式及风格相关音阶建议。
- 吉他、贝斯、鼓三轨音量和静音控制。
...
- Chord、Jam 和练习曲目列表均按需加载；Jam 的 UI 节拍回调去重，本地编排写入采用延迟合并。
...
src/chords/         和弦数据、多把位、指板图、扫弦试听和切换训练
src/jam/            Jam 段落、Fill、调性建议、存储、风格与音频引擎
src/practice/       节拍器练习曲目数据与筛选界面
